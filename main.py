...
class InsSectionHandler(webapp2.RequestHandler):
		...
		params = {
			'displayName' : "prinya-course-" + course_id + "-" + str(section_number)
		}
		result = service_plus.circles().insert(userId="me",body=params).execute(http=http)
                ...
                sql="INSERT INTO section (regiscourse_id,section_number,teacher_id,capacity,enroll, circle_id, circle_url) \
                    	...
                    	(SELECT staff_id FROM staff WHERE type=2 AND firstname='%s' AND university_id='%s'),'%d','0', '%s', '%s')"%(course_id,section_number,teacher,session['university_id'],capacity, str(result['id']), str(result['selfLink']))
                cursor.execute(sql);
                conn.commit();
                conn.close();
